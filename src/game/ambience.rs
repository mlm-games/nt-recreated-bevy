...
mod tests {
    ...
    #[test]
    fn route_areas_map_to_music() {
        assert_eq!(music_for_area(AreaId::Desert), MusicCue::Desert);
        assert_eq!(music_for_area(AreaId::Sewers), MusicCue::Sewers);
        assert_eq!(music_for_area(AreaId::Scrapyards), MusicCue::Scrapyards);
        assert_eq!(music_for_area(AreaId::CrystalCaves), MusicCue::CrystalCaves);
        assert_eq!(music_for_area(AreaId::FrozenCity), MusicCue::FrozenCity);
        assert_eq!(music_for_area(AreaId::Labs), MusicCue::Labs);
        assert_eq!(music_for_area(AreaId::Palace), MusicCue::Palace);
    }

    #[test]
    fn secret_areas_map_to_music() {
        assert_eq!(music_for_area(AreaId::Oasis), MusicCue::Oasis);
        assert_eq!(music_for_area(AreaId::PizzaSewers), MusicCue::PizzaSewers);
        assert_eq!(music_for_area(AreaId::CursedCaves), MusicCue::CursedCaves);
        assert_eq!(music_for_area(AreaId::Jungle), MusicCue::Jungle);
        assert_eq!(music_for_area(AreaId::Vault), MusicCue::Vault);
        assert_eq!(music_for_area(AreaId::CrownVault), MusicCue::CrownVault);
        assert_eq!(music_for_area(AreaId::HQ), MusicCue::Hq);
        assert_eq!(music_for_area(AreaId::City), MusicCue::City);
    }

    #[test]
    fn area_ambience_maps_are_stable() {
        assert_eq!(ambience_for_area(AreaId::Desert), AmbienceCue::DesertWind);
        assert_eq!(ambience_for_area(AreaId::Sewers), AmbienceCue::SewerDrip);
        assert_eq!(ambience_for_area(AreaId::Labs), AmbienceCue::LabBuzz);
        assert_eq!(ambience_for_area(AreaId::HQ), AmbienceCue::HqSirens);
        assert_eq!(
            ambience_for_area(AreaId::Campfire),
            AmbienceCue::CampfireCrackle
        );
    }

    #[test]
    fn loop_bosses_reuse_boss_themes() {
        assert_eq!(
    ...
    #[test]
    fn throne_chain_uses_unique_themes() {
        assert_eq!(
    ...
    #[test]
    fn non_boss_enemies_do_not_override_music() {
        assert_eq!(boss_music_for_kind(EnemyKind::Bandit), None);
        assert_eq!(boss_music_for_kind(EnemyKind::IdpdGrunt), None);
        assert_eq!(boss_music_for_kind(EnemyKind::Rat), None);
        assert_eq!(boss_music_for_kind(EnemyKind::Hyper), None);
    }

    #[test]
    fn campfire_music_is_explicit() {
        assert_eq!(music_for_area(AreaId::Campfire), MusicCue::Campfire);
        assert_eq!(
            ambience_for_area(AreaId::Campfire),
            AmbienceCue::CampfireCrackle
        );
    ...
    #[test]
    fn ambience_none_has_no_candidates() {
        assert!(ambience_candidates(AmbienceCue::None).is_empty());
    }

    #[test]
    fn every_music_cue_has_at_least_one_candidate() {
        let all = [
            MusicCue::Desert,
            MusicCue::Sewers,
            MusicCue::Scrapyards,
        ...
        for cue in all {
            assert!(
                !music_candidates(cue).is_empty(),
                "cue {cue:?} unexpectedly had no candidate paths"
            );
        }
    }

    #[test]
    fn every_ambience_cue_but_none_has_candidates() {
        let all = [
            AmbienceCue::DesertWind,
            AmbienceCue::SewerDrip,
            AmbienceCue::ScrapHum,
        ...
        for cue in all {
            assert!(
                !ambience_candidates(cue).is_empty(),
                "cue {cue:?} unexpectedly had no candidate paths"
            );
        }
    }
