...
mod tests {
    ...
    fn run_at(loop_count: u32, area: AreaId, floor: u32) -> Run {
        Run {
            loop_count,
            area,
            floor,
    ...
    #[test]
    fn raids_require_loop() {
        let run = run_at(0, AreaId::Desert, 16);
        assert!(!should_trigger_idpd(&run, 0, 25, false));
    }

    #[test]
    fn raids_require_low_enemy_count() {
        let run = run_at(1, AreaId::Desert, 16);
        assert!(!should_trigger_idpd(&run, 9, 25, false));
        assert!(should_trigger_idpd(&run, 3, 25, false));
    }

    #[test]
    fn raids_suppressed_in_hq_and_secrets() {
        let hq = run_at(2, AreaId::HQ, 27);
        assert!(!should_trigger_idpd(&hq, 0, 25, false));

        let vault = run_at(2, AreaId::Vault, 9);
        assert!(!should_trigger_idpd(&vault, 0, 25, false));
    }

    #[test]
    fn pending_wave_blocks_retrigger() {
        let run = run_at(1, AreaId::Desert, 16);
        assert!(!should_trigger_idpd(&run, 0, 25, true));
    }

    #[test]
    fn game_over_blocks_raids() {
        let mut run = run_at(1, AreaId::Desert, 16);
        run.game_over = true;
        assert!(!should_trigger_idpd(&run, 0, 25, false));
    }

    #[test]
    fn edge_points_stay_near_bounds() {
        let pts = edge_spawn_points_away_from(Vec2::ZERO);
        for p in pts {
            let on_x_edge = (p.x.abs() - (ARENA_W * 0.5 - 56.0)).abs() < 0.1;
            let on_y_edge = (p.y.abs() - (ARENA_H * 0.5 - 56.0)).abs() < 0.1;
            assert!(on_x_edge || on_y_edge, "{p:?}");
        }
    }

    #[test]
    fn edge_points_order_farthest_first() {
    ...
    #[test]
    fn wave_escalation_matches_pressure() {
        // Low pressure: light.
        assert_eq!(choose_wave(1, 5, 0), RaidWave::Light);
        // Mid pressure: medium unless the rare van roll lands.
        assert_eq!(choose_wave(1, 15, 2), RaidWave::Medium);
        assert_eq!(choose_wave(1, 15, 5), RaidWave::VanDrop);
        // High pressure: heavy/van rotation.
        assert_eq!(choose_wave(3, 20, 4), RaidWave::VanDrop);
        assert_eq!(choose_wave(3, 20, 2), RaidWave::Heavy);
        assert_eq!(choose_wave(3, 20, 1), RaidWave::Medium);
    }
}
