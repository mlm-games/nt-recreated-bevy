...
//! generated SFX. No assets from the original game are used.
    ...
    fn build(&self, app: &mut App) {
        app.init_resource::<Score>()
            ...
            .add_systems(
                Update,
                (
                    // Always-on (even while paused) UI sync + mutation picker.
                    sync_hud,
                    ...
                    // Save flush runs whenever dirty and state is InGame.
                    progress_sys::flush_dirty_save.run_if(in_state(AppState::InGame)),
                ),
            )
            .add_systems(
