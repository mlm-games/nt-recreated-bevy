...
mod tests {
    ...
    #[test]
    fn surface_boundary_is_inclusive() {
        assert!(point_in_zone(
            Vec2::new(16.0, -16.0),
            Vec2::ZERO,
            Vec2::splat(16.0),
    ...
    #[test]
    fn cobweb_has_priority_over_ice() {
        let zones = [
            (
                Vec2::ZERO,
                SurfaceZone {
        ...
        assert_eq!(
            surface_at_point(Vec2::ZERO, zones),
            Some(SurfaceKind::Cobweb),
        );
    }

    #[test]
    fn no_surface_outside_all_zones() {
        let zones = [(
            Vec2::ZERO,
            SurfaceZone {
                kind: SurfaceKind::Ice,
        ...
        assert_eq!(surface_at_point(Vec2::new(40.0, 0.0), zones), None);
    }

    #[test]
    fn cobweb_reduces_velocity() {
        let before = Vec2::new(200.0, 0.0);
        let after = surface_velocity(SurfaceKind::Cobweb, before, 1.0 / 60.0, 0.82, 240.0);

        assert!(after.length() < before.length());
    }

    #[test]
    fn cobweb_caps_actor_speed() {
        let after = surface_velocity(
            SurfaceKind::Cobweb,
            Vec2::new(1000.0, 0.0),
            1.0 / 60.0,
            0.82,
        ...
        assert!(after.length() <= 200.0 * 0.52 + 0.01);
    }

    #[test]
    fn ice_preserves_more_momentum_than_regular_friction() {
        let before = Vec2::new(160.0, 0.0);
        let ordinary = before * 0.82;
        let ice = surface_velocity(SurfaceKind::Ice, ordinary, 1.0 / 60.0, 0.82, 240.0);

        assert!(ice.length() > ordinary.length());
        assert!(ice.length() < before.length());
    }

    #[test]
    fn ice_has_a_speed_cap() {
        let ice = surface_velocity(
            SurfaceKind::Ice,
            Vec2::new(2000.0, 0.0),
            1.0 / 60.0,
            0.82,
        ...
        assert!(ice.length() <= 200.0 * 1.28 + 0.01);
    }

    #[test]
    fn toxic_barrel_has_explosion_and_hazard() {
        let effect = PropDeathEffect::toxic_barrel();

        assert!(effect.explosion.is_some());
        assert!(effect.hazard.is_some());
        assert_eq!(effect.hazard.unwrap().kind, EnvironmentHazardKind::Toxic,);
    }

    #[test]
    fn car_explosion_is_larger_than_barrel() {
        let car = PropDeathEffect::car().explosion.expect("car explosion");
        let barrel = PropDeathEffect::legacy_barrel()
            .explosion
            .expect("barrel explosion");

        assert!(car.radius > barrel.radius);
        assert!(car.damage > barrel.damage);
    }

    #[test]
    fn mine_has_fire_residue() {
        let mine = PropDeathEffect::mine();
        let hazard = mine.hazard.expect("mine hazard");

        assert_eq!(hazard.kind, EnvironmentHazardKind::Fire);
    }

    #[test]
    fn environment_position_rejects_arena_edge() {
        assert!(valid_environment_position(Vec2::ZERO, 32.0));
        assert!(!valid_environment_position(Vec2::new(ARENA_W, 0.0), 32.0));
    }
}
