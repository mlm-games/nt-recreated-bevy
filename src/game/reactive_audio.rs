...
mod tests {
    ...
    const ALL_CUES: &[ReactiveCue] = &[
        ...
        ReactiveCue::UiClick,
        ReactiveCue::UiBack,
        ReactiveCue::UiConfirm,
        ReactiveCue::UiCycle,
    ];

    #[test]
    fn every_cue_has_candidates() {
        for cue in ALL_CUES {
            assert!(
                !cue_candidates(*cue).is_empty(),
                "{cue:?} has no candidates"
            );
        }
    }

    #[test]
    fn first_stinger_is_allowed() {
        assert!(throttle_allows(ReactiveCue::LevelUp, None, 0.0));
    }

    #[test]
    fn immediate_repeat_is_blocked() {
        assert!(!throttle_allows(ReactiveCue::LevelUp, Some(1.0), 1.02));
    }

    #[test]
    fn repeat_after_window_is_allowed() {
        let window = cue_throttle_seconds(ReactiveCue::LevelUp);
    ...
    #[test]
    fn kill_cue_is_quieter_than_boss_cue() {
        assert!(cue_base_volume(ReactiveCue::Kill) < cue_base_volume(ReactiveCue::BossAppear));
    }

    #[test]
    fn kill_throttle_is_short() {
        assert!(cue_throttle_seconds(ReactiveCue::Kill) < 0.2);
    }

    #[test]
    fn ui_cycle_maps_from_weapon_cycle() {
        assert_eq!(
            ui_action_to_cue(&UiAction::CycleStartWeapon(1)),
            Some(ReactiveCue::UiCycle),
        );
    }

    #[test]
    fn ui_confirm_maps_from_start_game() {
        assert_eq!(
            ui_action_to_cue(&UiAction::StartGame),
            Some(ReactiveCue::UiConfirm)
        );
    }

    #[test]
    fn slider_actions_stay_silent() {
        assert_eq!(ui_action_to_cue(&UiAction::SetMasterVol(0.5)), None);
        assert_eq!(
            ui_action_to_cue(&UiAction::SetLanguage("en".to_string())),
            None,
        );
    }

    #[test]
    fn quit_maps_to_back() {
        assert_eq!(
            ui_action_to_cue(&UiAction::QuitApp),
            Some(ReactiveCue::UiBack)
        );
        assert_eq!(
            ui_action_to_cue(&UiAction::QuitToTitle),
            Some(ReactiveCue::UiBack)
        );
    }

    #[test]
    fn combat_intensity_quiet_room_is_silent() {
        assert_eq!(
            combat_intensity_target(combat_intensity_score(0, 0, 0)),
            0.0,
        );
    }

    #[test]
    fn combat_intensity_boss_alone_is_audible() {
        let score = combat_intensity_score(1, 0, 1);
        assert!(combat_intensity_target(score) > 0.0);
    }

    #[test]
    fn idpd_counts_more_than_normal_enemy() {
        let normal = combat_intensity_score(5, 0, 0);
        let idpd = combat_intensity_score(5, 3, 0);
        assert!(idpd > normal);
    }

    #[test]
    fn intensity_target_monotonic() {
        let mut prev = 0.0;
        for score in 0..20u32 {
            let now = combat_intensity_target(score);
            assert!(now >= prev, "{score}: {now} < {prev}");
            prev = now;
        }
    }
    ...
    fn smoothing_moves_toward_target() {
        let next = smooth_value(0.0, 1.0, 1.0 / 60.0, 0.5);
        assert!(next > 0.0);
        assert!(next < 1.0);
    }

    #[test]
    fn smoothing_reaches_target_over_time() {
        let mut value = 0.0;
        for _ in 0..240 {
            value = smooth_value(value, 1.0, 1.0 / 60.0, 0.4);
        }
        assert!(value > 0.98);
    }

    #[test]
    fn smoothing_handles_zero_dt() {
        assert_eq!(smooth_value(0.3, 1.0, 0.0, 0.5), 1.0);
    }

    #[test]
    fn intensity_candidates_exist_for_main_areas() {
        for area in [
            AreaId::Desert,
            AreaId::Sewers,
            AreaId::Scrapyards,
            ...
            assert!(!intensity_candidates(area).is_empty(), "{area:?}");
        }
    }

    #[test]
    fn intensity_candidates_absent_for_secret_limbo() {
        assert!(intensity_candidates(AreaId::Campfire).is_empty());
        assert!(intensity_candidates(AreaId::Vault).is_empty());
    }

    #[test]
    fn kill_streak_fires_every_ten() {
        let mut state = ReactiveAudioState::default();

        let mut fired = 0;
        for i in 0..30 {
            if state.note_kill(i as f32 * 0.1) {
                fired += 1;
            }
        }
    ...
    #[test]
    fn kill_streak_resets_after_gap() {
        let mut state = ReactiveAudioState::default();

        for i in 0..5 {
            assert!(!state.note_kill(i as f32 * 0.1));
        }

        assert!(!state.note_kill(10.0));

        for i in 1..9 {
            assert!(!state.note_kill(10.0 + i as f32 * 0.1));
        }

        assert!(state.note_kill(10.9));
    }
}
